...
In Sense mode a row stands for one Meaning of one Entry, or for the whole Entry when its sense is zero.
...
A Meaning is named by its title, or by its definition when it has no title, or by the unknown label when it has neither.
The words for the whole Entry and for an unknown name are handed in, because the record has no window to read a key from.
...
A row that names itself as its parent is skipped rather than followed, so a bad row cannot loop the walk.
