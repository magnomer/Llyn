...
Reads the held entry's rows, the morphology switch, and whether a fetch is running, and a failed read empties the box.
...
An empty row therefore says the form is looked up once the entry is saved, unless a fetch started elsewhere still runs.
