...
A video is played from a card and knows nothing of that tray, so the two cannot be wired to each other directly.
...
A value outside it is pulled to the nearest end rather than refused, since a slider is the only thing that sets it.
