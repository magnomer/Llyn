...
One source: its name, small, in the same fixed column the pronunciation menu gives it, so the two menus line up.
...
The hover highlight is conditioned on the row being takeable and idle, so neither an unreachable source nor a running download lights up as a choice.
