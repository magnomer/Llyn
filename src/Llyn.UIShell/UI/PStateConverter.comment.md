...
Every panel that must know whether a value is unknown asks here rather than reading the state itself.

## `public static bool PStateConverterCheck(LState state)`

Whether a bare state is the unknown one.
The other two overloads unwrap a value or a mark and ask this.

## `public static bool PStateConverterCheck(LStateValue? value)`

Whether a stored value was marked as not known.
A missing value is not unknown, since nothing was recorded for it.

## `public static bool PStateConverterCheck(LStateMark? mark)`

Whether a payload-free state, such as the author credit of a Source, was marked as not known.
