...
With the switch on it asks the engine to fetch.
The engine declines by itself when nothing is missing or a fetch already runs.
It then asks whether a fetch is running.
An empty row can then say it is being looked up rather than lost.
