...
### `IReadOnlyList<LMarkupEntry> entries = await Task.Run(() => _lEngine.LEngineMarkupRead(path));`

The file is read once so the customs window can list what it declares.
A long file is parsed away from the panel's own thread, so the window keeps drawing.
...
### `LMarkupOutcome outcome = await Task.Run(() => _lEngine.LEngineMarkupImport(path, intakes));`
