...
This is the recovery of last resort, not a substitute for migration: `LSchemaMigration` still carries a database forward whenever it can.
...
Returns what had to be done, so the caller can tell the user their old data is no longer in front of them.

Only a SQLite fault or a refused schema version is treated as an unusable database.
Those are the two ways a broken or unreadable file announces itself.
Anything else — a missing folder, a locked file, a denied path — is a workspace problem that resetting would not fix and would hide, so it is left to propagate.
...
A pooled connection holds the file open even after the code that used it disposed it, and Windows refuses to move a file that is open.

The write-ahead log and shared-memory files move with the database.
They belong to it, and one left behind would be read as part of the clean database that takes its name.
...
A counter is added when the stamp alone is taken, because two rescues in the same second would otherwise name the same file and the second would erase the first.
