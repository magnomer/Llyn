...
A built in Register is named by its language and the key its language pack gave it, and that pair is unique.
The pack is re-read on every load of its language, and the unique pair is what turns the second read into a no-op.
