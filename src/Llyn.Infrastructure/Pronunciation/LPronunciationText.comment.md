...
It strips any inner markup and returns the first non-empty phonetic form.
...
Returns the first non-empty normalized phonetic found by `openPattern` across all matches in `html`.
...
Decodes HTML entities and strips enclosing IPA delimiters, so the caller stores just the phonetic characters.
