...
So are `region`, `split`, `remark`, `until` and `folded`.
They are read as the rule's place, its cut, its remark pattern, its remark bound and its fold.
`recast` is read like `rewrite` but over the note, and `superscript` raises the note's digits.
