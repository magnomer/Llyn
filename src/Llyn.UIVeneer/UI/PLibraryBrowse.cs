...
public partial class PLibrary
{
    ...
    {
        if (_pLibraryVista is not LVista vista)
        {
            PSieveMark.Visibility = Visibility.Collapsed;
            return;
        }

        PSieveMark.Visibility = vista.LVistaFiltered ? Visibility.Visible : Visibility.Collapsed;
    }
    ...
    internal void PLibraryScribeRestore(bool editing)
    {
        if (editing)
        {
            if (_pLibraryVista?.LVistaChosen is null)
            {
                return;
            }
        }
