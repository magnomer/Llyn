...
public partial class PArticulation : UserControl
{
    ...
    private ScrollViewer PArticulationLane => (ScrollViewer)FindName(nameof(PArticulationLane));

    private Grid PArticulationRack => (Grid)FindName(nameof(PArticulationRack));

    private Border PVowelChart => (Border)FindName(nameof(PVowelChart));

    private Border PConsonantChart => (Border)FindName(nameof(PConsonantChart));

    private Grid PVowel => (Grid)FindName(nameof(PVowel));

    private Grid PConsonant => (Grid)FindName(nameof(PConsonant));
    ...
    private void PArticulationHeaderPlace(Grid table, string key, int column)
    {
        ...
        header.Style = (Style)PArticulationRack.FindResource("Articulation.Header");
    ...
    {
        TextBlock side = new();
        side.Style = (Style)PArticulationRack.FindResource("Articulation.Side");
    ...
    private void PArticulationCellPlace(Grid table, string characters, int column, int row)
    {
        ...
        foreach (string character in characters.Split(' ', System.StringSplitOptions.RemoveEmptyEntries))
        {
            ...
            {
                Content = character,
                Style = (Style)PArticulationRack.FindResource("Articulation.Glyph")
            };
