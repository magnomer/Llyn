...
Writes a measuring twin from its field: the text, or the hint while the field is empty.
It replaces a style binding and trigger, so the twin still closes the cell on the text.
