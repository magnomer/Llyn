...
Loads the control's markup from the Veneer and wears it as its content.
The markup's name scope is copied onto the control, so its named parts answer `FindName`.

## `private StackPanel PColophonBody`
