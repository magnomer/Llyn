...
## `private void PCitationApply(FrameworkElement container, object item, string? _)`

Fills one offered Source from its item, the work its bindings did before.
The press is subscribed on the row through the transcript dictionary's forwarder, removed first so it never doubles.

## Inline notes

### `PCitationSheet.MinWidth = anchor.ActualWidth;`

The drawer is at least as wide as the field it opens under, which a binding held before.
