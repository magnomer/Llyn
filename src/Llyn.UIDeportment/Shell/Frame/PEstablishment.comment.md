...
Loads the bar's markup from the Veneer and wears it as its content.
The markup's name scope is copied onto the bar, so its named parts answer `FindName`.

## `private StackPanel PEstablishmentPending`

The named parts are read through the name scope, so the code reads them as it read the generated fields.
