...
public class PEstablishment : UserControl
{
    ...
    public PEstablishment()
    {
        UserControl surface = (UserControl)System.Windows.Application.LoadComponent(
            new Uri("/Llyn.UIVeneer;component/Shell/Frame/PEstablishment.xaml", UriKind.Relative));
        Content = surface;
        NameScope.SetNameScope(this, NameScope.GetNameScope(surface));
    }

    private StackPanel PEstablishmentPending => (StackPanel)FindName(nameof(PEstablishmentPending));

    private TextBlock PEstablishmentUnsaved => (TextBlock)FindName(nameof(PEstablishmentUnsaved));

    private TextBlock PEstablishmentEntry => (TextBlock)FindName(nameof(PEstablishmentEntry));

    private TextBlock PEstablishmentSize => (TextBlock)FindName(nameof(PEstablishmentSize));
