...
## `public sealed record LCandidate(string LCandidateSource, string? LCandidatePhonetic, int LCandidateOrder, bool LCandidateReached)`

What one lookup source had to say about a headword.
Every source produces exactly one, so the menu shows a row for each and none is silently absent.
