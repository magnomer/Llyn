...
Every writer takes its text from here, so no two formats disagree about an unreadable field.

## `public static string LPortraitTextRead(LStateValue? value, string mark)`

A specified value shows its text, an unreadable one shows the mark, and an unset one shows nothing.
