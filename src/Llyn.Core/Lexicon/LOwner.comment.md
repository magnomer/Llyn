...
Which kind of stored row an id names when a seam takes the referring side of a reference. An Example, a Tag and a Situation are independent data several kinds of row may point at, and an Example and a Situation each cite a Source of their own, so a seam that reads, attaches or detaches one of them needs the caller to say which side the id it carries belongs to.
