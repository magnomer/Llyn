...
On commit a row is stored whenever any of its seven texts is filled.
...
- `LReflexDraftRomanization` — The reading in a romanization scheme, such as pinyin or Pe̍h-ōe-jī, or empty.
- `LReflexDraftMeaning` — The lexical meaning paired with the reading, or empty.
- `LReflexDraftOwned` — True when the user entered the meaning and a source rebuild must preserve it.
- `LReflexDraftNote` — What the source says of the reading, such as `literary`, or empty.
...
True when the row carries no language, kind, text, romanization, meaning, note or region.
Such a row has nothing worth storing.
