...
- `LCardDraftImage` — The Images the card references, each carrying the location it is loaded from — a file on this machine or a web address — and what is known about it, in the order they are shown. A Video the form also carries is not here and never reaches the store: a video is watched while the entry is being written, not kept with it.
