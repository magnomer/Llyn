...
`LRevisionChangeTargetId` and `LRevisionChangeTargetType` name what was touched.
...
- `LRevisionChangeTargetId` — Id of the row the change touched, recorded and not a reference.
- `LRevisionChangeTargetType` — Entity type of the target, for example `entry` or `sense`.
