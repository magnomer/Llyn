...
public sealed class TDraftCommitting
{
    ...
    public void ExampleCommit_FreshSentence_RecordsRevision()
    {
        ...
        LRevisionDelta change = Assert.Single(workspace.TRevisionChangeRead(revision.Value));
        Assert.Equal("example", change.LRevisionDeltaSubject);
        Assert.Equal("create", change.LRevisionDeltaKind);
        Assert.Equal(example.LExampleId, change.LRevisionDeltaTarget);
    }
