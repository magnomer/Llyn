...
public sealed class TTenureObserver
{
    ...
    private sealed class TTenureNotice(List<LBulletin> notices) : LObserver
    {
        public void LObserverBulletinHandle(LBulletin bulletin) => notices.Add(bulletin);
    }
    ...
    public void Prepare_RequestApplied_ReturnsNewestDraftAndDropsItsNotice()
    {
        ...
        tenure.TTenureDraftAttach(LSubject.LSubjectDraft, new TTenureNotice(_tTenureNotices));
        LDraft? draft = tenure.TTenurePrepare(() =>
        {
            tenure.TTenureRequestApply(TInterface.TExampleTextCreate(tenure.LTenureId, TInterface.TStateValueCreate("new text")));
    ...
    public void Attach_SubjectAndDraftFilters_DeliverOnlyMatchingNotices()
    {
        ...
        tenure.TTenureObserverAttach(LSubject.LSubjectSettings, new TTenureNotice(_tTenureNotices));
        tenure.TTenureDraftAttach(LSubject.LSubjectDraft, new TTenureNotice(_tTenureNotices));
        ...
        Assert.Equal([LSubject.LSubjectSettings, LSubject.LSubjectDraft], _tTenureNotices.Select(row => row.LBulletinSubject));
    ...
    public void EntryAttach_StoredEntryIdentity_IsDistinctFromDraftIdentity()
    {
        ...
        tenure.TTenureEntryAttach(LSubject.LSubjectFrequency, new TTenureNotice(_tTenureNotices));
