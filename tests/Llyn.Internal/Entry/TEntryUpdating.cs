...
public sealed class TEntryUpdating
{
    ...
    public void EntryUpdate_DraftNamesItsCards_KeepsThemRecordsChanges()
    {
        ...
        IReadOnlyList<LRevisionDelta> changes = workspace.TRevisionChangeRead(revision);
        Assert.Contains(changes, change =>
            change.LRevisionDeltaKind == "delete" && change.LRevisionDeltaSummary == "second");
        Assert.Contains(changes, change =>
            change.LRevisionDeltaKind == "update" && change.LRevisionDeltaTarget == firstId);
        Assert.Contains(changes, change =>
            change.LRevisionDeltaKind == "update" && change.LRevisionDeltaTarget == thirdId);
        Assert.Contains(changes, change =>
            change.LRevisionDeltaKind == "create" && change.LRevisionDeltaSummary == "fourth");
