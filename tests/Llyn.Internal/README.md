...
The suite is split by platform into two projects that share the namespace `Llyn.Tests`.
`Llyn.Internal` targets `net10.0` and holds every test that runs without Windows.
`Llyn.Windows` targets Windows and holds `Deportment/` and every other test that compiles only there.
It reuses this project's relays, fakes and workspace, so the relay layer is not duplicated.
...
- `Conduct/` — the gates both drivers call.
- `Deportment/`, in `Llyn.Windows` — the GUI driver over its Windows controls.
