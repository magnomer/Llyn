...
Generation 7 holds every project-specific value in `TAuditSetting.cs`.
That generated sidecar is the only file here that names a project.
Every other file in this project is identical in every project at this generation.
`TAuditSetting.cs` is generated and committed, so this project builds on its own.

Generation 7 checks missing prefix, component count, base registration, and verb ending.
...
It does not audit the tooling's own files.
