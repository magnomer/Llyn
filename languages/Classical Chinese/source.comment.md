...
The romanization, meaning and source note follow it in their own cells.
...
The `split` key cuts a text and a romanization listing several readings into one row per reading.
...
The `gloss` key reads the source note and optional parenthesized meaning of one reading.
It is looked for after the match by romanization, as far as the `until` key, here the next language header.
So a Hakka reading never takes the gloss of a Southern Min reading further down.
...
The last word of a pair is the reading and everything before it the meaning.
So 강 이름 호 gives the reading 호 with the meaning 강 이름.
...
`{meaning} {text}` reads 희롱할 롱(농).
The clip key names the pattern cut out of it, here the 두음법칙 form in brackets.
So the list reads 弄 [희롱할 롱].

## `reflex[1]` Mandarin

The Mandarin rule reads the Sinological IPA of each Standard Chinese pronunciation block with its Hanyu Pinyin as the romanization.
Its region is Beijing.
The first block on the page is the primary pronunciation and so the one marked in common use.

## `reflex[2]` Cantonese

The Cantonese rule reads the Sinological IPA of each Standard Cantonese block whole with its Jyutping line as the romanization.
Its region is Guangzhou.
The split parts a block listing two readings, so `lʊŋ²²` under `lung⁶` and `nʊŋ²²` under `nung⁶` are two rows.
The superscript key raises the tone digit of the Jyutping romanization.
The first row is again marked in common use.
The Jyutping romanization allows no tag but sup.
...
Older spellings follow in sup, and their historical or ancient label is read ahead as the source note.
...
The bracketed style label is captured as the note of that row.
...
A block with one unlabelled IPA line gives one row with no note, marked as the first.
A block listing two readings is parted by the split into two rows.
The superscript key raises every tone digit of the romanization, so a Nanchang `lung5` is stored as `lung⁵`.
Wugniu writes the tone before the syllable, as `7oq`.
The Wu recast moves that leading digit to the end before it is raised, so the romanization becomes `oq⁷`.
The gloss is still looked for under `7oq`, the form the page writes.
The gloss of each reading is read from the box under the block.
`literary` becomes the note with an empty meaning.
